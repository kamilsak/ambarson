...
$(document).ready(function(){
	...
	$("#nereyeIl").change(function() {
		var ill = $(this).val();
	$.ajax({
		type : 'GET',
		url : "/ortak/ilceler?ad="+ill,
		success : function(data) {
			var nereye = $('#nereyeIlce'), option = "";
			nereye.empty();
			for (var i = 0; i < data.length; i++) {
				 option = option + "<option value='"+data[i].ad + "'>"
						+ data[i].ad + "</option>";
			}
			nereye.append(option);
		},
		error : function() {
			toastr.error("Hata", "Eksik seçim.!", {
				allowHtml:true,
	...
	$("#neredenIl").change(function() {
		var ill = $(this).val();
	$.ajax({
		type : 'GET',
		url : "/ortak/ilceler?ad="+ill,
		success : function(data) {
			var nereden = $('#neredenIlce'), option = "";
			nereden.empty();
			for (var i = 0; i < data.length; i++) {
				 option = option + "<option value='"+data[i].ad + "'>"
						+ data[i].ad + "</option>";
			}
			nereden.append(option);
		},
		error : function() {
			toastr.error("Hata", "Eksik seçim.!", {
				allowHtml:true,
