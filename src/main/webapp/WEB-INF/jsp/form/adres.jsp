	...
	<div class="panel-heading">
		<strong> <c:choose>
				<c:when test="${empty adres.id}">
					...
					<c:if test="${neyi.equals('islemAl')}">Alıcı</c:if><c:if test="${neyi.equals('islemGon')}">Gönderici</c:if> Adres Kaydı
				</c:when>
				<c:otherwise>
	...
	<form:form method="post" class="form-horizontal" action="adres/add?neyi=${neyi}" modelAttribute="adres"
		...
		<div class="panel-body">
			...
			<div class="form-group">
				...
				<div class="col-md-4">
					<form:select class="form-control" id="selectSubcat" path="ilce">
						<option value="-1">-Select-</option>
					</form:select>
				</div>
			</div>
		...
		<div class="panel-footer">
			<c:choose>
				<c:when test="${neyi.equals('islemAl') || neyi.equals('islemGon')}">
					<button value="save" class="btn btn-xs btn-default">
						<span class="glyphicon glyphicon-ok"></span>
						<c:choose>
							<c:when test="${empty adres.id}"> Devam et</c:when>
